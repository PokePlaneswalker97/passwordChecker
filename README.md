...
This program runs a password strength checker on a String inputted by a user. This input is not checked properly for inject attacks (as I understand them). Once a String is input by the user (via enter/return), the String is then checked for length and use of various regEx characters to see if the password is more or less secure. As far as I'm aware, the only dependency to run this code is the ability to run python code.
